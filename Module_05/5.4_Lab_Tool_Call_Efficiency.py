...
# MAGIC 1. **Tool-Using Agent** — Build a small agent with `get_weather` + `get_time` tools, traced as TOOL spans
# MAGIC 2. **`ToolCallEfficiency`** — Built-in agent judge that flags redundant or wasteful tool invocations
# MAGIC 3. **Custom Tool-Count Scorer** — Trace-based `@scorer` that counts TOOL spans and enforces a hard SLA
# MAGIC 4. **`KnowledgeRetention`** — Multi-turn judge that checks whether the agent remembers facts across turns
# MAGIC 5. **v1 vs v2 Comparison** — Quantify behaviour change after a system-prompt rewrite — efficiency up, correctness held
# MAGIC 6. **Module 5 Outcome** — Confirm: full RAG eval (5.2) + agent behaviour eval (5.4) covers all three quality dimensions
# MAGIC
# MAGIC ## Prerequisites
# MAGIC - Lab 1.3 (workspace + Foundation Model API)
# MAGIC - Lab 4.2 + 4.3 (judges + `@scorer` patterns)
# MAGIC - Conceptual familiarity with span types from Lab 5.3
# MAGIC
...
# MAGIC
# MAGIC ### 1. Why Agent-Specific Judges
# MAGIC - Final-answer judges miss path quality — a correct answer that took 8 tool calls is still expensive and fragile.
# MAGIC - Agent judges read the trace and grade *how* the agent reached the answer, not just whether it did.
# MAGIC
# MAGIC ### 2. Decision Matrix — Agent Eval
# MAGIC - **Redundant tool calls?** → `ToolCallEfficiency` (built-in agent judge).
# MAGIC - **Hard SLA on tool count?** → custom trace-based `@scorer` counting TOOL spans.
# MAGIC - **Multi-turn fact retention?** → `KnowledgeRetention` (built-in multi-turn judge).
# MAGIC - **Path-vs-destination question?** → compare versions in MLflow Experiments comparison view.
# MAGIC
# MAGIC ### 3. Module 5 Outcome — Coverage Map
# MAGIC - **RAG quality (5.2)** — retrieval recall + groundedness + correctness, with failure-mode diagnosis.
# MAGIC - **Agent behaviour (5.4)** — tool efficiency + multi-turn retention + version-over-version regression catch.
# MAGIC - Together: end-to-end coverage for both data-grounded and tool-using agents.
# MAGIC
# MAGIC ### Next Steps
