...
# MAGIC 1. **`client_request_id` Contract** — Tag every trace with the UI-supplied request ID — the join key for your `/api/feedback` endpoint
# MAGIC 2. **Trace Lookup** — Resolve `client_request_id` → `trace_id` with `mlflow.search_traces(filter_string=…)`
# MAGIC 3. **`mlflow.log_feedback`** — Attach typed `Feedback` assessments with `AssessmentSource(source_type="HUMAN", source_id=…)`
# MAGIC 4. **Multi-Field Feedback** — Helpful + free-text rationale + numeric satisfaction — multiple assessments per trace
# MAGIC 5. **Human vs. Machine View** — Compare aggregate human feedback against `Correctness`/`Safety` judge scores on the same cohort
# MAGIC 6. **Production-Shaped Endpoint** — Wrap the lookup + log_feedback into a `submit_user_feedback(...)` function
# MAGIC
# MAGIC ## Prerequisites
# MAGIC - Lab 1.3 (workspace + experiment)
# MAGIC - Lab 4.2 (built-in judges)
# MAGIC - Lab 6.5 (registered scorers — useful for the comparison view)
# MAGIC
...
# MAGIC
# MAGIC ### 1. Why the Loop Matters
# MAGIC - Human feedback is the ground truth that calibrates everything else (Lab 7.3).
# MAGIC - The contract is small — `client_request_id` flows from UI → trace tag → feedback endpoint.
# MAGIC - Once the loop is closed, every product event becomes labelled training data for free.
# MAGIC
# MAGIC ### 2. Three Things to Get Right
# MAGIC - **Stable `name`** — column name in eval results; don't rename across product versions.
# MAGIC - **Correct `source`** — `AssessmentSource(source_type="HUMAN", source_id=user_id)`.
# MAGIC - **One assessment per fact** — separate `log_feedback` calls for helpful, satisfaction, comment.
# MAGIC
# MAGIC ### 3. From Raw Feedback to Calibration
# MAGIC - Aggregate human feedback alongside automated judge scores in MLflow.
# MAGIC - Disagreement between humans and judges is the signal Lab 7.3 uses to refine rubrics.
# MAGIC
# MAGIC ### Next Steps
# MAGIC - **Lab 7.3** — judge calibration loop: measure agreement, refine the rubric, re-evaluate.
# MAGIC