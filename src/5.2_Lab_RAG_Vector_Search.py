...
# MAGIC
# MAGIC **Goal:** Build a minimal but real RAG agent using Databricks Vector Search + Foundation Model API, then evaluate **all three quality pillars**: retrieval, generation, and end-to-end correctness.
# MAGIC
# MAGIC By the end of this lab you will have:
# MAGIC 1. Created a **Vector Search index** over Delta Lake doc chunks in Unity Catalog
# MAGIC 2. Built a **traced RAG agent** — retrieval span → LLM generation span, both visible in MLflow
# MAGIC 3. Instrumented with `mlflow.openai.autolog()` so retrieval and generation appear as separate spans
# MAGIC 4. Run **`RetrievalGroundedness` + `Correctness` + `retrieved_document_recall`** (custom code scorer) in one `evaluate()`
# MAGIC 5. Diagnosed failure modes — distinguishing **retrieval miss** (Correctness=2, Groundedness=5) from **hallucination** (Correctness=2, Groundedness=2)
# MAGIC
# MAGIC > **Why this lab matters:** RAG systems fail in two very different ways and the fix is different for each. The judges + retrieval recall scorer together pinpoint *which* stage is the culprit on each row. That's the whole point of three-pillar evaluation.
# MAGIC
# MAGIC > **Prereq:** Lab 1.3 (workspace + UC + FM API), Lab 2.2 (`tutorial_eval_v1` dataset), Lab 4.2/4.3 (built-in judges + code scorers). Vector Search must be enabled in your workspace.

# COMMAND ----------

# MAGIC %pip install --quiet "mlflow[databricks]>=3.1" databricks-openai databricks-vectorsearch
...
# DBTITLE 1,Aggregate three-pillar pass rates
display(results.tables["eval_results"].selectExpr(
